...
def format_number(curr_num, match_num):
    ...
    # Make sure the decimal type in 'tickSize' ("." or ",")
    if "." in match_num_string:
        ...
        # If match_num is an str(integer), format the number to that quantity step
        # Fix error: Order size must adhere to quantity step of 10
        return str(round(float(curr_num_string) / float(match_num_string)) * float(match_num_string)) # debug
        # return f"{int(curr_num)}"
 
# Format time (Python datetime format -> ISO string for API)
...
def get_ISO_times():
    ...
    # Get timestamps (for 400 hours data in total)
